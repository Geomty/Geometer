    ...
    execute(message, args, Discord, client, querystring, fetch, togglespamArray) {
        ...
        if (togglespamArray[0].indexOf(message.guild.id) != -1) {
            ...
            if (user) {
                ...
                if (member) {
                    if (message.content.slice(10, message.content.length) === '<@!704355591686062202>') {
                        return message.channel.send('Hey! Stop trying to make me abuse my creator!');
		            } else if (message.content.slice(10, message.content.length) === '<@!744626726222692389>') {
			            return message.channel.send('Hey! Stop trying to make me abuse myself!');
                    } else {
                        for (g = 0; g < 100; g++) {
                            message.channel.send(`<@!${user.id}> HAHA GET SPAM PINGED`);
                        }
                    }
                ...
                message.channel.send('Correct Usage: =spamping <user>');
            }
        }
    }
}
